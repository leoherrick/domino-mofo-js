...
function Ai(player){
	...
	// private vars
	var player = player;
	var board = player.match.current_game.board;
	var hand = player.hand;
	
	// private methods
	this.playable_dominoes = function(){
		var result = new DominoGroup();
			board.suits_in_play.forEach(function(suit){
				hand.find_all_dominoes_of_suit(suit).forEach.(function(domino){
					result.push(domino);
				});
			});
...
Ai.prototype.has_play? = function(){
	this.playable_dominoes.length > 0 ? return true : return false
}
