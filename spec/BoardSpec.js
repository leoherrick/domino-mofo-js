describe("Board", function() {
	...
	describe("#lead_out", function(){
		it("should add domino to board", function(){
			board = new Board
			board.lead_out(new Domino(1,2))
			expect(board.length).toEqual(1)
		})
		it("should promote to spinner if is double", function(){
			board = new Board
			board.lead_out(new Double(2))
			expect(board.length).toEqual(1)			
			expect(board.spinner).toBeTruthy()			
		})
	})
...
TEST_BOARDS = {
	board_with_11_spin_and_12_13_14: function(){
		board = new Board
		board.lead_out(new Double(1))
		board.play_domino_on_board_by_suit(new Domino(1,2), board.find_domino_with_suits(1,1), 1)
		board.play_domino_on_board_by_suit(new Domino(1,3), board.find_domino_with_suits(1,1), 1)
		board.play_domino_on_board_by_suit(new Domino(1,4), board.find_domino_with_suits(1,1), 1)
		return board
	},
	...
	board_with_12_25_56: function(){
		board = new Board
		board.lead_out(new Domino(1,2))
		board.play_domino_on_board_by_suit(new Domino(2,5), board.find_domino_with_suits(1,2), 2)
		board.play_domino_on_board_by_suit(new Domino(5,6), board.find_domino_with_suits(2,5), 5)
		return board
	}	
}